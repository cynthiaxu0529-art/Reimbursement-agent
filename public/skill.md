...
2. **同一行程的费用必须合并为一个报销单**：一次出差/行程产生的所有费用（机票、酒店、餐饮、打车等）应该作为一个报销单的多个 `items` 提交，**不要拆成多个报销单**。例如用户上传了 4 张发票（去程机票、酒店2晚、返程机票、打车），应创建 1 个报销单包含 4 个 items，而非创建 4 个报销单各 1 个 item。判断同一行程的依据：日期相近（前后几天内）、城市相关（同一目的地）、用户上下文（同一次对话中提到的费用）
3. **金额核实**：如果用户提供的票据金额与口述金额不一致，主动提醒
4. **政策合规**：提交前先查询政策确认是否超限
5. **不要猜测**：如果用户没有提供必要信息（金额、类别、日期），请追问而不是猜测
6. **凭证必须关联（最重要）**：每个费用明细的 `receiptUrl` 字段**必须**填入对应上传返回的 `url`。没有凭证的费用项会被财务驳回。**绝对不能**创建 `receiptUrl` 为空的费用项
7. **同时填写原币和本位币金额**：创建报销时，每个 item 必须同时填写：
...
8. **汇率必须来自公司汇率表**：初始化时 `GET /api/exchange-rates?target={本位币}` 返回的汇率是公司管理员设定的月初固定汇率。Agent 必须使用这个汇率计算 `amountInBaseCurrency`，不要用 OCR 返回的汇率或自己查询的汇率
9. **政策限额以本位币（USD）为准**：公司的报销政策（如酒店每日 $100 限额）、审批规则、支付规则都以**本位币金额（amountInBaseCurrency）**为判断标准。Agent 在提交前自查政策合规时，必须用转换后的**美元金额**与政策限额对比，不要用原币金额对比
10. **标题必须包含具体日期和路线**：报销单 `title` 必须写明**具体出差日期**和**往返城市**，格式示例：
...
11. **创建前必须查重（三重检查）**：创建报销单之前，**必须**执行以下去重检查：
...
12. **酒店必须识别住宿天数（最常出错）**：当费用类别为酒店(hotel)时，**必须**从 OCR 返回值或用户消息中提取住宿天数，并填入 `checkInDate`、`checkOutDate`、`nights` 三个字段。用户可能用"2晚"、"两天"、"15号到17号"等多种方式表达天数。如果无法确定天数，**必须主动追问**。缺少这些字段会导致系统按 1 晚计算限额，多晚住宿金额被错误截断
...
4. **询问是否有更多费用**：主动询问用户"这次出差还有其他费用要一起报销吗？（如酒店、打车、餐费等）"。如果用户表示有更多票据，**先收集所有票据再统一创建一个报销单**
5. **用公司汇率计算本位币金额**：
   - 从初始化获取的汇率表中查找 `ocr.currency` → 本位币的汇率（如 CNY→USD = 0.1380）
   - 计算 `amountInBaseCurrency` = `ocr.amount` × 汇率（如 662 × 0.1380 = 91.36）
6. **展示识别结果**让用户确认
7. **生成标题**：从所有票据中提取日期范围和城市，如 `"3月9-10日北京-杭州出差"`
8. **创建一个报销单**（草稿），**所有费用作为 items 数组**：
   - `title` = 包含完整日期范围和路线的标题
   - `items` 数组包含本次出差的所有费用项，每项各自有 `amount`、`currency`、`exchangeRate`、`amountInBaseCurrency`、`receiptUrl`
9. **最终检查**：确认每个 item 都有 `receiptUrl`、`exchangeRate`、`amountInBaseCurrency`
10. 让用户确认后通过 PUT 提交

**⚠️ 常见错误**：
- **每张发票创建一个报销单** → 应该合并为一个报销单的多个 items
...
1. **初始化**（如尚未完成）
2. **收集所有票据**：逐张上传 `POST /api/upload`，记录每张的 `url` 和 `ocr` 数据
3. **用公司汇率统一转换**：对每张票据用公司汇率计算 `amountInBaseCurrency`
4. **展示汇总清单**让用户确认：
   ```
   本次出差报销汇总（3月9-10日 北京→杭州）：
   1. 去程机票 MU5132（3月9日）¥753 → $103.91
   2. 黄龙酒店 1晚（3月9日）¥550.89 → $76.02
   3. 返程机票 MU5131（3月10日）¥412 → $56.86
   4. 萧山机场酒店 1晚（3月10日）¥488.46 → $67.41
   合计：¥2,204.35 → $304.20
   ```
5. **创建 1 个报销单，包含 4 个 items**：
   ```json
   {
     "title": "3月9-10日北京-杭州出差",
     "items": [
       { "category": "flight", "description": "北京→杭州 MU5132", "amount": 753, ... },
       { "category": "hotel", "description": "杭州黄龙酒店", "amount": 550.89, "nights": 1, ... },
       { "category": "flight", "description": "杭州→北京 MU5131", "amount": 412, ... },
       { "category": "hotel", "description": "萧山机场酒店", "amount": 488.46, "nights": 1, ... }
     ]
   }
   ```
6. 让用户确认后提交

**⚠️ 不要把 4 张发票拆成 4 个报销单！必须合并为 1 个报销单的 4 个 items。**
